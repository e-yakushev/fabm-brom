...
!
! !INTERFACE:
   module fabm_niva_brom_metals_hg
!
! !DESCRIPTION:
!
! !USES:
...
! !PUBLIC DERIVED TYPES:
   type,extends(type_base_model),public :: type_niva_brom_metals_hg
...
!      type (type_state_variable_id)        :: id_Ni,id_Ni2S,id_Ni_prt
...
!
! !INPUT PARAMETERS:
   class (type_niva_brom_metals_hg), intent(inout), target :: self
...
! Parameters, i.e. rate constants
   ...
   call self%get_parameter(self%O2s_nf, 'O2s_nf', '[uM O]','half saturation for nitrification',default=4.488_rk)
      !---- Hg---------!
   call self%register_state_variable(self%id_Hg0, 'Hg0',  'mol/m**3','Hg(0)',       minimum=0.0_rk)
   call self%register_state_variable(self%id_Hg2, 'Hg2',  'mol/m**3','Hg(II)',       minimum=0.0_rk)
   call self%register_state_variable(self%id_MeHg,'MeHg', 'mol/m**3','MeHg',  minimum=0.0_rk)
   call self%register_state_variable(self%id_HgS, 'HgS',  'mol/m**3','HgS', minimum=0.0_rk)
...
!
! !INPUT PARAMETERS:
   class (type_niva_brom_metals_hg),intent(in) :: self
